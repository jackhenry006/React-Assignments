...
const Usercards = ({ show, setToggle }) => {
  return (
    <div className="w-80 bg-white rounded-2xl shadow-xl overflow-hidden border border-indigo-100   duration-300">
      {/* Image */}
      <img className="w-full h-56 object-cover" src={show.image} alt="User" />

      {/* Details */}
      <div className="p-5">
        <h1 className="text-2xl font-bold text-slate-800">{show.name}</h1>

        <p className="text-slate-500 mt-2">{show.email}</p>

        <p className="text-slate-500 mt-1">📞 {show.mobile}</p>
